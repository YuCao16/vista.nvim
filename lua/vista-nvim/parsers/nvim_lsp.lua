    ...
    for index, _ in ipairs(line) do
      ...
      if config.show_guides then
        ...
        elseif index == #line then
          -- add fold markers
          if config.fold_markers and folding.is_foldable(node) then
          ...
          elseif depth > 1 then
            if node.isLast then
              ...
              add_guide_hl(
                running_length,
                running_length + vim.fn.strlen(render.markers.bottom) - 1
              )
            else
              line[index] = render.markers.middle
              add_guide_hl(
                running_length,
                running_length + vim.fn.strlen(render.markers.middle) - 1
              )
            end
          end
        ...
        elseif not node.hierarchy[index] and depth > 1 then
          ...
          add_guide_hl(
            ...
            running_length + vim.fn.strlen(render.markers.vertical) - 1 + 2 * marker_space
          )
        end
      end
