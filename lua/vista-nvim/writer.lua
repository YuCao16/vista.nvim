...
local function is_buffer_vista(bufnr)
  ...
  return string.match(name, ".*VistaNvim_.*") ~= nil and ft == "VistaNvim" and isValid
end
...
function M.add_highlighs_title(bufnr, theme)
  ...
  if theme == "tree" then
    vim.api.nvim_buf_add_highlight(bufnr, hlns, "@string", 0, max_title_width(), -1)
  elseif theme == "type" then
    vim.api.nvim_buf_add_highlight(bufnr, hlns, "@type", 0, max_title_width(), -1)
  end
end
...
M._title_highlight_ns = M._title_highlight_ns
  or vim.api.nvim_create_namespace("vista-icon-highlight")
    ...
    for k, v in pairs(hl_info) do
      if not vim.tbl_isempty(v) then
        for group, scope in pairs(v) do
          vim.api.nvim_buf_add_highlight(bufnr, ns_id, group, k, scope[1], scope[2])
        end
      end
    end
    return
...
function M.write_title_loading(bufnr)
  ...
  vim.api.nvim_buf_set_lines(bufnr, 0, 0, false, {
    config.fold_markers[2] .. " " .. M.clean_path(view.View.current_filepath),
  })
...
function M.write_title(bufnr, switch, width)
  ...
  elseif switch then
    ...
    vim.api.nvim_buf_set_option(bufnr, "modifiable", true)
    local current_title = vim.api.nvim_buf_get_lines(view.View.bufnr, 0, 1, false)[1]:sub(1, -6)
      .. theme_marker
...
function M.write_title_width(bufnr)
  ...
  vim.api.nvim_buf_set_lines(bufnr, 0, 1, false, {
    config.fold_markers[2]
      .. " "
      .. M.clean_path(M.current_filepath, M.current_width)
      .. " "
      .. theme_marker,
  })
  vim.api.nvim_buf_set_option(bufnr, "modifiable", false)
