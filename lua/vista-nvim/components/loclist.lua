...
function Loclist:new(o)
    o = vim.tbl_deep_extend(
        "force",
        vim.deepcopy(Loclist.DEFAULT_OPTIONS),
        o or {},
        {
...
function Loclist:add_item(item)
    if not self.groups[item.group] then
        self.groups[item.group] =
            { is_closed = self.groups_initially_closed or false }
    end
...
function Loclist:draw_group(
    ...
    if with_label then
        ...
        table.insert(
            section_hl,
            { self.highlights.group, #section_lines, 0, #line }
        )
        if self.show_group_count then
            table.insert(
                section_hl,
                { self.highlights.group_count, #section_lines, #line, -1 }
            )
            local total = #group
            if total > 99 then
                total = "++"
            ...
            for _, i in ipairs(item.right) do
                if i ~= nil and i.text ~= nil then
                    ...
                    if i.hl then
                        table.insert(
                            temp_hl,
                            { i.hl, #section_lines, #line + #temp_line, -1 }
                        )
                    else
                        table.insert(
                            temp_hl,
