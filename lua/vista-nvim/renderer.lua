...
local function build_section_separator(section, index)
    ...
    if type(config.section_separator) ~= "function" then
        utils.echo_warning(
            "'section_separator' must be string, table or function"
        )
        return
    end
...
local function build_section_title_separator(section, index)
    ...
    if type(config.section_title_separator) ~= "function" then
        utils.echo_warning(
            "'section_title_separator' must be false, string, table or function"
        )
        return
    end
...
local function get_lines_and_hl(sections_data)
    ...
    for index, data in ipairs(sections_data) do
        ...
        local section_title_separator =
            build_section_title_separator(data.section, index)

        local section_content_start = #lines
        ...
        local section_lines, section_hl =
            expand_section_lines(data.lines, #lines)
