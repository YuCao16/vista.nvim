...
local function update_hl(group, tbl)
  ...
  local new_hl = vim.tbl_extend("force", { bg = old_hl.bg, fg = old_hl.fg }, tbl)
...
function M.add_hover_highlight(bufnr, line, col_start)
  vim.api.nvim_buf_add_highlight(bufnr, M.hovered_hl_ns, "VistaFocusedSymbol", line, col_start, -1)
end
...
function M.setup()
  ...
  if vim.fn.hlexists("FocusedSymbol") == 0 then
    ...
    vim.api.nvim_set_hl(0, "FocusedSymbol", { bg = cline_hl.background, fg = string_hl.foreground })
  end
