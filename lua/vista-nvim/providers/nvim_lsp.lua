      ...
      if err then
        on_info(err, {
          contents = {
            kind = "markdown",
            content = {
              "Error retrieving hover information: " .. tostring(err),
            },
          },
        })
      else
  ...
  if not success then
    on_info(err, {
      contents = {
        kind = "markdown",
        content = {
          "Failed to request hover information: " .. tostring(err),
        },
      },
    })
  end
...
function M.request_symbols(on_symbols, bufnr)
  bufnr = bufnr or 0
  vim.lsp.buf_request_all(bufnr, "textDocument/documentSymbol", getParams(bufnr), on_symbols)
end
